...
$().ready(function() {
	...
	$goodsTypeId.change( function() {
		if (previousGoodsTypeId != "") {
			$.dialog({type: "warn", content: "修改充值卡类型后当前“属性参数”数据将会丢失,是否继续!", width: 450, ok: "确 定", cancel: "取 消", modal: true, okCallback: goodsTypeChange, cancelCallback: goodsTypeReset});
		} else {
			goodsTypeChange();
		}
	...
	$goodsForm.validate({
		...
		messages: {
			"goods.goodsCategory.id": "请选择充值卡分类",
			"goods.name": "请填写充值卡名称",
			"goods.goodsSn": "充值卡编号已存在",
	...
	// 表单验证
	$.validator.addMethod("goodsImageFileListRequired", $.validator.methods.required, "请选择上传充值卡图片");
	$.validator.addMethod("goodsImageFileListImageFile", $.validator.methods.imageFile, "充值卡图片格式错误");
	$.validator.addMethod("goodsImageOrderListDigits", $.validator.methods.digits, "充值卡图片排序必须为零或正整数");
		...
		<form id="goodsForm" action="<#if isAddAction>goods!save.action<#else>goods!update.action</#if>" method="post" enctype="multipart/form-data">
			...
			<table id="infoTable" class="inputTable tabContent">
				...
				<tr class="specificationDisabledInfo">
					...
					<td>
						<input type="text" name="goods.storePlace" class="formText" value="${(goods.storePlace)!}" title="用于记录充值卡所在的具体仓库位置,便于检索" />				 						
					</td>
				</tr>
				<tr>
					<th>
			...
			<div class="tabContent">
				<table id="specificationTable" class="inputTable">
					<tr class="noneHover">
						<td colspan="9">
							<label class="red">
								<@checkbox id="isSpecificationEnabled" name="goods.isSpecificationEnabled" value="${(goods.isSpecificationEnabled)!false}" />启用充值卡规格
							</label>
						</td>
					</tr>
					<tr class="title">
