...
$().ready( function() {
	...
	$.memberVerify = function () {
		if(getCookie("memberUsername") != null) {
			...
			$.ajax({
				url: xxs.base + "/shop/member!ajaxMemberVerify.action",
				type: "POST",
				dataType: "json",
				async: false,
				cache: false,
	...
	if ($header.size() > 0) {
		...
		$.showLoginWindow = function (redirectUrl) {
					...
					<table>
						...
						<tr>
							...
							<td>
								...
								<a href="' + xxs.base + '/shop/member!passwordRecover.action">忘记了密码? 点击找回!</a>
							</td>
						</tr>
					</table>
				</form>';
			...
			function login() {
				...
				$.ajax({
					url: xxs.base + "/shop/member!ajaxLogin.action",
		...
		$.showRegisterWindow = function () {
			...
			function register() {
				...
				$.ajax({
					url: xxs.base + "/shop/member!checkUsername.action",
					...
					success: function(data) {
						if (data == "true") {
							$.ajax({
								url: xxs.base + "/shop/member!checkReferrer.action",
								...
								success: function(data) {
									if (data == "true") {
										$.ajax({
											url: xxs.base + "/shop/member!ajaxRegister.action",
	...
	$.addFavorite = function(id) {
		...
		$.ajax({
			url: xxs.base + "/shop/favorite!ajaxAdd.action",
			data: {"id": id},
			type: "POST",
			dataType: "json",
			cache: false,
	...
	$.addCartItem = function(id, quantity) {
		...
		$.ajax({
			url: xxs.base + "/shop/cart_item!ajaxAdd.action",
	...
	if ($comment.size() > 0) {
		...
		var ajaxUrl = xxs.base + "/shop/comment!ajaxSave.action";
		
		$commentReply.click( function() {
			var $this = $(this);
			forCommentId = $this.attr("forCommentId");
			$forCommentId.val(forCommentId);
			ajaxUrl = xxs.base + "/shop/comment!ajaxReply.action";
			$sendTitle.html("回复评论");
			$sendComment.show();
		});
		...
		$sendComment.click( function() {
			forCommentId = null;
			$forCommentId.val("");
			ajaxUrl = xxs.base + "/shop/comment!ajaxSave.action";
			$sendTitle.html("发表评论");
			$sendComment.hide();
		});
