<ul>
	<p>欢迎您!&nbsp;&nbsp;<span class="username">${loginMember.username}</span>&nbsp;&nbsp;[<a class="userLogout" href="member!logout.action"">退出</a>]</p>
		<p>当前账户余额: <span class="red"> ${loginMember.deposit}</span> 元</p>
		<p>处理中的金额: <span class="red"> ${loginMember.memberRank.name}</span></p>
		<p>当前可用余额: <span class="red"> ${loginMember.memberRank.name}</span></p>
